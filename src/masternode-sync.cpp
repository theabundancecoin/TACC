...
void CMasternodeSync::Process()
{
    ...
    if (IsSynced()) {
        ...
        */
        if (mnodeman.CountEnabled() == 0 && !isRegTestNet && 0) {
            Reset();
        } else
            return;
    }
    ...
    if (RequestedMasternodeAssets == MASTERNODE_SYNC_FAILED && lastFailure + (1 * 60) < GetTime()) {
        LogPrint(BCLog::MASTERNODE, "CMasternodeSync::Process() - SYNC FAILED retry");
        Reset();
    } else if (RequestedMasternodeAssets == MASTERNODE_SYNC_FAILED) {
        LogPrint(BCLog::MASTERNODE, "CMasternodeSync::Process() - SYNC FAILED retry but not so early");
        return;
    }
...
bool CMasternodeSync::SyncWithNode(CNode* pnode)
{
    ...
    if (RequestedMasternodeAssets == MASTERNODE_SYNC_SPORKS) {
        if (pnode->HasFulfilledRequest("getspork")){
            LogPrint(BCLog::MASTERNODE, "CMasternodeSync::Process() already fullfiled sporks");
            GetNextAsset();
            return true;
        }
