// Copyright (c) 2017-2020 The TaccCoin developers
    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select zTACC Denominations to Spend"));
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < nRequiredConfs)
                ...
                strReason = "Your wallet is locked. Impossible to spend zTACC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zTACC seed used to mint this zPIV is not the same as currently hold in the wallet";
            else
