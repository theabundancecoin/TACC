// Copyright (c) 2019-2020 The TaccCoin developers
    ...
    ui(new Ui::PrivacyWidget)
{
    ...
    // Only Convert to TACC enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzTACC->setTitleClassAndText("btn-title-grey", tr("Total 0 zTACC"));
    ui->btnTotalzTACC->setSubTitleClassAndText("text-subtitle", tr("Show denominations of z owned."));
    ui->btnTotalzTACC->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", tr("Coin Control"));
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", tr("Select TACC outputs to mint into z."));
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", tr("Reset Spent zTACC"));
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", tr("Reset zerocoin database."));

    connect(ui->btnTotalzTACC, &OptionButton::clicked, this, &PrivacyWidget::onTotalZpivClicked);
...
void PrivacyWidget::onMintSelected(bool isMint)
{
    QString btnText;
    if (isMint) {
        btnText = tr("Mint zTACC");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of TACC to mint into zTACC"));
    } else {
        btnText = tr("Convert back to TACC");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of z to convert back into TACC"));
    }
...
void PrivacyWidget::onTotalZpivClicked()
{
    ...
    if (!isVisible) {
        ...
        ui->btnTotalzTACC->setRightIconClass("btn-dropdown", true);
    } else {
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzTACC->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked()
{
    ...
    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zTACC is currently undergoing maintenance"));
        return;
    }
    ...
    if (!ctx.isValid()) {
        inform(tr("You need to unlock the wallet to be able to %1 zTACC").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value)
{
    ...
    if (!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)) {
        ...
        // Mint succeed
        inform(tr("zTACC minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value)
{
    ...
    if (!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zTACC converted back to TACC"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked()
{
    if (ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any TACC to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply()
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTACC </b>";
    ...
    ui->btnTotalzTACC->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
