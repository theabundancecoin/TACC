...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("TaccCoin Core client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("TaccCoin Core"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("TaccCoin Core"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
