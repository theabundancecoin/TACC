...
bool CMasternodePing::CheckAndUpdate(int& nDos, bool fRequireAvailable, bool fCheckSigTimeOnly)
{
    ...
    //strprintf("Masternode protocol too old %d - req %d", pmn->protocolVersion, ActiveProtocol());
    //LogPrint(BCLog::MNPING, ActiveProtocol());
    if (!isMasternodeFound) {
        LogPrint(BCLog::MNPING, "CCCCCC");
    }
    if (isMasternodeFound && pmn->protocolVersion >= ActiveProtocol()) {
        LogPrint(BCLog::MNPING, "XXXXXXX");
