...
CAmount GetBlockValue(int nHeight)
{
    ...
    nHeight--;
    if (nHeight == 0) {
        nSubsidy = 1990000000 * COIN;
    }
    else if (nHeight < 55000 && nHeight > 0) {
        nSubsidy = 0.01 * COIN;
    }
    else if (nHeight <= 98199 && nHeight >= 55000) {
        nSubsidy = 10 * COIN;
    }
    else if (nHeight <= 227799 && nHeight >= 98200) {
        nSubsidy = 6 * COIN;
    }
    else if (nHeight <= 357399 && nHeight >= 227800) {
        nSubsidy = 4 * COIN;
    }
    else if (nHeight <= 573399 && nHeight >= 357400) {
        nSubsidy = 3 * COIN;
    }
    else {
        nSubsidy = 2 * COIN;
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    if (!IsBlockValueValid(pindex->nHeight, nExpectedMint, nMint)) {
        return state.DoS(100, error("ConnectBlock() : reward pays too much (actual=%s vs limit=%s  block=%s)",
                                    FormatMoney(nMint), FormatMoney(nExpectedMint), pindex->nHeight),
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired && block.IsProofOfWork()) {
